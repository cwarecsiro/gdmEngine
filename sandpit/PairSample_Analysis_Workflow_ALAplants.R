...
## Plot results ###############################################
library(ggplot2)
parameter.results.tbl<-read.csv('//osm-23-cdc/OSM_CBR_LW_DEE_work/processing/biol/vascular_plants/SitePairSampleTesting/Plants_parameters_results.csv')
figures.out.folder<-"//osm-23-cdc/OSM_CBR_LW_DEE_work/processing/biol/vascular_plants/SitePairSampleTesting/Figures"
...
#############################################################
## Assess evenness in the sampled dissimilarities
library(DescTools)
parameter.results.tbl<-read.csv('//osm-23-cdc/OSM_CBR_LW_DEE_work/processing/biol/vascular_plants/SitePairSampleTesting/Plants_parameters_results.csv')
analysis.out.folder<-"//osm-23-cdc/OSM_CBR_LW_DEE_work/processing/biol/vascular_plants/SitePairSampleTesting"
analysis.out.files<-as.character(list.files(analysis.out.folder))
for(i.run in 1:nrow(parameter.tbl))
  {
  #find if there is an output file to match the run.name
  #paste(as.character(parameter.results.tbl$run.name[i.run]),"_",sep='')
  f.name <- analysis.out.files[pmatch(paste(as.character(parameter.results.tbl$run.name[i.run]),"_",sep=''), analysis.out.files)]
  if(!is.na(f.name))
  {
    # open the file
    load(file.path(analysis.out.folder,f.name))
    # catch the dissimilarities used to fit the final model
    used.dissim = GDM_Builder_results$Mean.Final.GDM$observed
    # create a histogram of the dissimilarities
    dissim.hist <- hist(used.dissim, breaks=seq(from=0, to=1, by=0.025))
    Gini(dissim.hist$counts)#quantifies inequality, so high values are less even, zero = perfect evenness
    # remove the results
    rm(GDM_Builder_results)
  }# end if !is.na(f.name)
} # end for i.run
