...
// NNLSDoubleCore.cpp
//
// This version uses 8 byte doubles
//
...
int h12_( integer *mode, integer *lpivot, integer *l1, integer *m, 
...
static integer c__1 = (long long)1;
...
int h12_( int *mode, int *lpivot, int *l1, int *m, 
		  doublereal *u, int *iue, doublereal *up, doublereal *c__, 
...
int nnls_( doublereal *a, integer *mda, integer *m, integer *n, 
		   doublereal *b, doublereal *x, doublereal *rnorm, doublereal *w, doublereal *zz, integer *index, int *mode)
{
...
/*     NEAR LINEAR DEPENDENCE. */
    ...
    i__1 = npp1 + 1;
    h12_(&c__1, &npp1, &i__1, m, &a[j * a_dim1 + 1], &c__1, &up, &dummy, &c__1, &c__1, &c__0);
    unorm = 0.;
    if (nsetp != 0) {
	i__1 = nsetp;
	for (l = 1; l <= i__1; ++l) {
...
/*     ------------------------------------------------------------------ */
/* Subroutine */ 
int nnls_( doublereal *a, int *mda, int *m, int *n, 
		   doublereal *b, doublereal *x, doublereal *rnorm, doublereal *w, doublereal *zz, int *index, int *mode)
{
    /* System generated locals */
    integer a_dim1, a_offset, i__1, i__2;
    doublereal d__1, d__2;
...
/*                        OR IF M COLS OF A HAVE BEEN TRIANGULARIZED. */
    ...
    if (iz1 > iz2 || nsetp >= *m) {
	//sprintf(aaa, "%d", gmIterCurrent);
	//Message("At Exit 1", aaa);
	goto L350;
    }
...
*/
    ...
    if (wmax <= 0.) {
	//sprintf(aaa, "iteration: %d  wmax: %lf", gmIterCurrent, wmax);
	//Message("At Exit 2", aaa);
	goto L350;
    }
    iz = izmax;
    j = index[iz];
...
int nnls_c(double* a, long long* mda, long long* m, long long* n, double* b, 
...
///////////////////////////////////////////////////////////////////////////////////////////////
//
//	The main function called from nnlsFITDouble() in GDMCore.cpp
//
//
int nnls_Weighted( double* a, long long* mda, long long* m, long long* n, double* b, double *pWghts,
	              double* x, double* rnorm, double* w, double* zz, long long* index, 
	              int* mode )
{
	//
...
//
//
int nnls_Weighted( double* a, int* mda, int* m, int* n, double* b, double *pWghts,
	              double* x, double* rnorm, double* w, double* zz, int* index, 
	              int* mode )
{
	//
