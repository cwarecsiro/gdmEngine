...
class TTranspactor
 ...
 public:
   ...
   int TransBinaryGridsToFloat(string arg_s_res_path);       // note names of transgrids are in s_binary_out
//  int TransBinaryGridsToFloat(string arg_s_res_path);
   void Refresh(void);
 ...
 private:
   ...
   string s_binary_in[30]; // full pathnames for all input grids (IN ALPHABETICAL ORDER of grid name)
   string s_binary_out[30]; // full pathnames for output transgrids as FLT
   ...
   int i_transtype[30]; //Type of transform for each layer: 0 set all to zero, 1 GDM transform, 2 Binary scaling
   int b_active[30]; //flag for active spline layers in the list.. e.g. constant grids, repeat grids
   double d_quant[30][5];  //Assume nobody uses >5 splines for overfitting reasons
   double d_coeff[30][5];  //Assume nobody uses >5 splines for overfitting reasons
   double d_m[30][2];  // slope parameter y=mx+c     [layer][bot or top]
   double d_c[30][2];  // constant parameter y=mx+c   [layer][bot or top]
