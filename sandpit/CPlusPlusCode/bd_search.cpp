...
void ANNbd_shrink::ann_search(ANNdist box_dist)
{
	...
	child[gmIN]->ann_search(inner_dist);	// search inner child first
	child[gmOUT]->ann_search(box_dist);	// ...then outer child
    }
    else {					// if outer box is closer
	child[gmOUT]->ann_search(box_dist);	// search outer child first
	child[gmIN]->ann_search(inner_dist);	// ...then outer child
    }
