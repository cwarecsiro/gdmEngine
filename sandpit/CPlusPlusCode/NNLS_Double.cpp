								...
								double *pRespVector, double *pDeviance, double *pWeights, FPTR fptr )
{
	...
	if (fptr)
	{
		fptr("Initialising for GDM regression...", nCurrent);
	}
	...
	for ( i=0; i<nRows; i++ )
	{
		if (fptr != NULL )
		{
			if (0 == i % 10000)  // increment progress bar every 10000 rows...
			{
	...
	if (fptr != NULL )
	{
		fptr("Performing GDM regression...", nCurrent);
	}
	for ( int nIter=0; nIter<nMaxIterations; nIter++ )
	{
		if (fptr != NULL )
		{
			if ( nIter * 100 / nMaxIterations )
			{
				nCurrent = nIter * 100 / nMaxIterations;
				fptr("Performing GDM regression...", nCurrent);
			}
		}
		...
		{
			_read( h, pTmp, (int)nRows * sizeof( double ) );
			pTmp += nRows;
		}
		_close( h );
...
//
// Do the WEIGHTED iterative NNLS regression process allowing for 
// residuals to be regressed with user defined coefficients held constant
//
// Takes a nRows X nCols matrix pEnvDataMatrix and a vector pRespVector
// of length nRows and iteratively produces a non-negative
// vector of coefficients representing the best fit of the matrix 
	...
	for ( int nIter=0; nIter<nMaxIterations; nIter++ )
	{
		...
		else
		{
			...
			for ( int i=0; i<nRows; i++ )
			{
				...
				for ( int j=0; j<nCols; j++ )
				{
					// apply the residual transformation to vector pNN
					if (pResidualVector[j] != -1.0 ) 
						dVal += pResidualVector[j] * pEnvDataMatrix[ ( j * nRows ) + i ];

					// use the regression coefficient
					else                             
						dVal += pCoeff[j] * pEnvDataMatrix[ ( j * nRows ) + i ];
				}
...
{
	GDM_INT nTotal = nRows * nCols;
	double *pNew = new double [ nTotal ];
	for ( GDM_INT i=0; i<nTotal; i++ )
	{
		pNew[i] = pMatrix[i];
	}
	return( pNew );
...
double *nnlsFITDouble( double *pEnvDataMatrix, GDM_INT nRows, GDM_INT nCols, double *pRespVector, double *pWeights )
{
	...
	GDM_INT *indx  = new GDM_INT [ nCols + 10 ];
	int mode;
...
double *WeightedNNLSRegression( char *lpTmpFile, 
								...
								double *pRespVector, double *pDeviance, double *pWeights, FPTR fptr )
{
	//Message("At Entry to WeightedNNLSRegression");
	int i;
	...
	for ( int nIter=0; nIter<nMaxIterations; nIter++ )
	{
		...
		//char aaa[9999];
		////sprintf(aaa, "%lf ", pZZ[0]);
		//sprintf(aaa, "Iter: %d   %lf ", nIter, pZZ[0]);
		//for (int a=1; a<171; a++)
		//{
		//	//sprintf(aaa, "%s%lf ", aaa, pZZ[a]);
		//	sprintf(aaa, "%s%lf ", aaa, pNN[a]);
		//}
		//Message(aaa, "pNN");
		...
		/*char aaa[4096];
		sprintf(aaa, "Intercept: %lf ", pCoeff[0]);
		for (int a=1; a<nCols; a++)
		{
			sprintf(aaa, "%s%lf ", aaa, pCoeff[a]);
		}
		Message(aaa);*/
		...
		dNewDev = CalcGDMDevianceDouble( pRespVector, pUU, pWeights, nRows );
		double dExitVal = ( dNewDev - dOldDev ) / ( dOldDev + dEpsilon );
		//Message(fabs( dExitVal ), "fabs( dExitVal )");
		//Message(dNewDev, "dNewDev");
