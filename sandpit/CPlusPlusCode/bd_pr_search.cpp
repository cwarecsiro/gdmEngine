...
void ANNbd_shrink::ann_pri_search(ANNdist box_dist)
{
	...
	if (child[gmOUT] != KD_TRIVIAL)		// enqueue outer if not trivial
	    ANNprBoxPQ->insert(box_dist,child[gmOUT]);
	child[gmIN]->ann_pri_search(inner_dist);	// continue with inner child
    }
    else {					// if outer box is closer
	if (child[gmIN] != KD_TRIVIAL)		// enqueue inner if not trivial
	    ANNprBoxPQ->insert(inner_dist,child[gmIN]);
	child[gmOUT]->ann_pri_search(box_dist);	// continue with outer child
    }
