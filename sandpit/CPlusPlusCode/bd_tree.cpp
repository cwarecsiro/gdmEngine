...
{
    child[gmOUT]->print(level+1, out);		// print out-child
    ...
    child[gmIN]->print(level+1, out);		// print in-child
}
	...
	ostream &out)			// output stream
{
    ...
    child[gmIN]->dump(out);			// print in-child
    child[gmOUT]->dump(out);			// print out-child
}
    ...
    ANNorthRect		&bnd_box)		// bounding box
{
    ...
    child[gmIN]->getStats(dim, ch_stats, inner_box);
    st.merge(ch_stats);				// merge them
						// get stats for outer child
    ch_stats.reset();				// reset
    child[gmOUT]->getStats(dim, ch_stats, bnd_box);
    st.merge(ch_stats);				// merge them
...
ANNdecomp selectDecomp(		// select decomposition method
    ...
    ANNorthRect		&inner_box)	// inner box if shrinking (returned)
{
    ANNdecomp decomp;			// decomposition
